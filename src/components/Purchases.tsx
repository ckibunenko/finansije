  ...
  return <section className="rounded-[28px] border border-white/60 bg-white/90 p-6 shadow-soft dark:border-slate-700 dark:bg-slate-900/80">
    ...
    <ul className="mt-4 divide-y divide-slate-100 dark:divide-slate-800">{rows.map((row) => <li className="py-4" key={row.id}>
      {editing?.id === row.id ? <form className="grid gap-3 sm:grid-cols-3" onSubmit={async (e) => {
        ...
        <div><p className="font-semibold text-brand-700 dark:text-sky-300">{formatCurrency(row.amount)}</p><p className="mt-1 text-sm dark:text-slate-200">{row.description || 'Kupovina'}</p><p className="mt-1 text-xs text-slate-500">{row.source === 'gpt' ? 'Uneto preko ChatGPT-ja' : row.source === 'import' ? 'Preneto iz stare evidencije' : row.source === 'shortcut' ? 'Uneto prečicom na telefonu' : row.source === 'viber' ? 'Uneto preko Vibera' : 'Uneto u aplikaciji'}</p></div>
        <div className="flex flex-wrap gap-2">
          <button type="button" className="secondary" disabled={busy} onClick={() => { setEditing(row); setAmount(String(Math.round(row.amount))); setNote(row.description); setEditDate(row.date); setError(''); setDeleting(null); }}>Izmeni</button>
