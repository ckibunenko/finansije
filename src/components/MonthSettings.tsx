...
export default function MonthSettings({ month, mutate, busy }: { month: MonthBudget; mutate: Mutate; busy: boolean }) {
  ...
  return <form className="grid gap-4 md:col-span-2 md:grid-cols-2" onSubmit={async (e) => {
    ...
    <label className="space-y-2 text-sm font-medium text-slate-600 dark:text-slate-300">Planirani mesečni budžet<input className="field" inputMode="numeric" value={budget} disabled={busy} onChange={(e) => { setBudget(e.target.value); setDirty(true); }} /></label>
    <label className="space-y-2 text-sm font-medium text-slate-600 dark:text-slate-300">Mesečni cilj za štednju<input className="field" inputMode="numeric" value={goal} disabled={busy} onChange={(e) => { setGoal(e.target.value); setDirty(true); }} /></label>
