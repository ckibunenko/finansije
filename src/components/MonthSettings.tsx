...
export default function MonthSettings({ month, mutate, busy }: { month: MonthBudget; mutate: Mutate; busy: boolean }) {
  const [budget, setBudget] = useState(String(Math.round(month.plannedMonthlyBudget)));
  const [goal, setGoal] = useState(String(Math.round(month.monthlySavingsGoal)));
  const [version, setVersion] = useState(month.version ?? 0);
  const [dirty, setDirty] = useState(false);
  const [error, setError] = useState('');
  useEffect(() => {
    if (!dirty) { setBudget(String(Math.round(month.plannedMonthlyBudget))); setGoal(String(Math.round(month.monthlySavingsGoal))); setVersion(month.version ?? 0); }
  ...
  return <form className="grid gap-4 md:col-span-2 md:grid-cols-2" onSubmit={async (e) => {
    ...
    <label className="space-y-2"><span className="text-sm font-medium text-slate-600 dark:text-slate-300">Planirani mesečni budžet</span><input className="field" inputMode="numeric" value={budget} disabled={busy} onChange={(e) => { setBudget(e.target.value); setDirty(true); }} /></label>
    <label className="space-y-2"><span className="text-sm font-medium text-slate-600 dark:text-slate-300">Mesečni cilj za štednju</span><input className="field" inputMode="numeric" value={goal} disabled={busy} onChange={(e) => { setGoal(e.target.value); setDirty(true); }} /></label>
